...
const payment = async (req, res) => {
    ...
    if (!process.env.STRIPE_KEY) {
        console.error("STRIPE_KEY is not set. Payment gateway not configured.");
        return res.status(500).send({ status: "error", msg: "Payment gateway not configured" });
    }
    ...
    try {
        ...
        status = "error";
        return res.status(500).send({ status: "error", msg: "Payment processing failed" });
    }
