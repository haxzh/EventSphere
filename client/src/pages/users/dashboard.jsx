...
function UserDashboard() {
    ...
    // If redirected back from Stripe Checkout with session_id, notify server to finalize registration
    useEffect(() => {
        const { session_id } = router.query;
        if (session_id) {
            (async () => {
                try {
                    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/checkout-complete`, {
                        method: "POST",
                    ...
                    if (res.ok && data.ok) {
                        alert("Payment confirmed and registration completed.");
                        // reload events or user dashboard data if needed
                        fetchAllEvents();
                    } else {
                        console.warn("Checkout completion not processed:", data);
                    }
                } catch (err) {
