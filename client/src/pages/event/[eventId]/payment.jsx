...
export default function payment() {
    ...
    const createCheckoutSession = async () => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/create-checkout-session`, {
                ...
                body: JSON.stringify({ price: product.price, name: product.name, eventId: event_id, user_token: user_id }),
            });
            if (!res.ok) {
    ...
    const isStripeConfigured = stripeKey && !stripeKey.includes("your_key") && stripeKey !== "";
            ...
            <div className="flex flex-col m-6 ">
                ...
                <div className="m-6 flex flex-col ">
                    ...
                    <div className="mt-4 flex items-center gap-4">
                        ...
                        {isStripeConfigured ? (
                            <div className="opacity-80 text-sm text-gray-600">or use direct card popup</div>
                        ) : (
                            <div className="text-sm text-gray-500 max-w-lg">
                                Direct tokenization popup disabled or unsupported; use the
                                "Pay via Stripe Checkout" button above.
                            </div>
                        )}
                    </div>
                </div>
